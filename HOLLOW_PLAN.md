...
- [ ] **Voice/video call STUN priority over TURN.** Audio/video calls sometimes pick TURN (relay) over STUN (direct P2P) due to ICE candidate race conditions — TURN relay candidates complete connectivity checks before STUN srflx pairs when getUserMedia latency delays the negotiation. Data channels, screen share, and Share consistently get STUN. Investigate: native-level ICE candidate priority tuning, `iceTransportPolicy` with ICE restart fallback, or delayed TURN candidate injection. Low priority — voice over TURN uses ~100kbps, negligible bandwidth impact at alpha scale.
