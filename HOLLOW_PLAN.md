...
- [ ] **ICE/STUN/TURN infrastructure**
  - [ ] Add `flutter_webrtc` package to Flutter dependencies
  - [ ] Deploy coturn (open-source TURN server) on OVH VPS alongside hollow-relay. Config: listening on 443/UDP+TCP, TLS cert from Let's Encrypt, shared secret auth
  - [ ] STUN: use public Google STUN servers (`stun:stun.l.google.com:19302`) + own coturn as backup STUN
  - [ ] TURN: own coturn server with time-limited credentials (HMAC-based, generated by hollow-relay on auth)
  - [ ] ICE configuration passed from Rust to Dart: list of STUN/TURN URLs + credentials

- [ ] **WebRTC signaling via WSS relay**
  - [ ] New `HavenMessage` variants: `WebRtcOffer { target_peer, sdp }`, `WebRtcAnswer { target_peer, sdp }`, `WebRtcIceCandidate { target_peer, candidate }`
  - [ ] Route signaling messages via `SendDirect` on WSS relay (existing infrastructure, just new message types)
  - [ ] Signaling is peer-to-peer targeted (not broadcast) — only the two peers establishing a connection see these messages

- [ ] **Peer connection manager (Rust)**
  - [ ] New module: `node/webrtc_manager.rs` — manages active RTCPeerConnection instances per peer
  - [ ] `establish_data_channel(peer_id)` — creates offer, sends via WSS signaling, waits for answer, returns data channel
  - [ ] Connection pooling: reuse existing data channel if already connected to peer (don't create new per-file)
  - [ ] Auto-close idle connections after 60s of no data transfer
  - [ ] Connection state tracking: `Connecting`, `Connected(direct)`, `Connected(relay/turn)`, `Failed`
  - [ ] Emit `NetworkEvent::PeerDataChannelReady { peer_id, is_direct: bool }` for UI status

- [ ] **Dart-side WebRTC integration**
  - [ ] `WebRtcService` class wrapping flutter_webrtc — handles RTCPeerConnection lifecycle
  - [ ] Listen for signaling messages from Rust (offer/answer/ice) and forward to flutter_webrtc
  - [ ] Send local ICE candidates back to Rust for relay forwarding
  - [ ] Data channel message handler: receives file chunks, forwards to Rust for reassembly
  - [ ] Note: flutter_webrtc runs in Dart; Rust handles signaling routing + file reassembly. Bridge via FFI events.

- [ ] **File streaming over data channels**
  - [ ] Modify `stream_to_peer()` in swarm.rs: if WebRTC data channel is ready for target peer → use it; else fall back to WSS relay binary (existing path)
  - [ ] Chunking: 64KB chunks over RTCDataChannel (WebRTC has ~256KB max message size, 64KB is safe + matches existing chunk size)
  - [ ] Progress tracking: reuse existing `StreamProgress` atomic counter + 500ms poll timer
  - [ ] Both directions: sender streams chunks, receiver reassembles to temp file (same as `ws_stream_transfer.rs` logic)
  - [ ] Graceful fallback: if data channel closes mid-transfer, automatically retry remaining chunks via WSS relay

- [ ] **Vault shard distribution over data channels**
  - [ ] Modify `VaultUploadFile` handler: distribute shards via data channel where available, WSS fallback for unreachable peers
  - [ ] Modify `ShardRequest`/`ShardResponse`: shard bytes via data channel, metadata via MLS (existing)
...
- [ ] **Connection quality indicators**
  - [ ] Show in member panel or peer tooltip: "Direct (P2P)" vs "Relayed (TURN)" vs "WSS Relay"
...
- [ ] **Testing & verification**
  ...
  - [ ] Test 3: Peer behind symmetric NAT (mobile hotspot) → should fall back to TURN
...
**Estimated scope:**
- New Dart: ~500 lines (WebRtcService, signaling bridge)
- New Rust: ~300 lines (webrtc_manager.rs, signaling message types, stream_to_peer modification)
- Modified: swarm.rs (signaling routing, stream_to_peer fallback), ws_stream_transfer.rs (data channel adapter)
- Infrastructure: coturn deployment on VPS (~30 min setup)

**Key insight:** This is NOT replacing the WSS relay. The relay stays for signaling, text messages, MLS encrypted ops, and CRDT sync. WebRTC data channels only handle the heavy stuff (files, shards, and later voice/video). The two systems complement each other — relay for reliability, WebRTC for bandwidth.

**After 5A is complete, Phase 5B (Voice & Video) becomes straightforward:**
- Same RTCPeerConnection already established for data channels
- Just add audio/video media tracks to the existing connection
- SFrame E2EE wraps the media tracks
- All the ICE/STUN/TURN infrastructure is already deployed and tested
...
- [ ] Fix "Connecting..." labels on Network column and DMs header
