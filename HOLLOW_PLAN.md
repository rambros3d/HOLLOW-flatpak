...
- [ ] **Integration test harness** — CI-based integration tests for runtime-dependent modules (`/node/`, `/storage/`, `/api/`, `/archive/`). Spin up relay in Docker, create test identities, connect peers, exercise sync/messaging/file transfer end-to-end. Currently excluded from Codecov because they need SQLCipher + relay + Flutter engine at runtime.
