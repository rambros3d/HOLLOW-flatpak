...
- [x] **Archive tab — personal data viewer + signed `.hollow-archive` export/import (part of "The RAT Files" protocol)**
  ...
  - [x] **Archive tab UI** — new top-level tab with icon on server strip / bottom bar (left of Downloads icon), replaces main screen when active (like Home tab today). Shell integration done: `archiveTabOpenProvider`, fade animation, Home deselects when archive is open.
    ...
    - [ ] **Future enhancements (post-launch):**
      - [ ] Export all server channels
      - [ ] Jump-to-date: date picker or scrollable timeline to navigate to a specific date in the chat history
      - [ ] Peer filter: filter messages by sender in channel archives (show only messages from a specific participant)
      - [ ] Search within archive: text search across the loaded archive's messages
...
- [ ] Count the chat messages sizes in the Server Storage inside servers
- [ ] Fix the Search bar in chats
- [ ] Add "disable animations" toggle in User Settings 
