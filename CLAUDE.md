...
- **CRITICAL — message signing must use `message_signing_payload()`.** ALL message types (DM, channel, file) MUST sign the canonical payload from `message_signing_payload("dm"|"ch", context, sender, ts, text)`, not the raw text. The `SendFile` path previously signed raw text (`"[file:abc123]"`) instead of the canonical format (`"haven-msg:dm:recipient:sender:ts:[file:abc123]"`), causing every file/image/video message signature to fail verification. Fixed Apr 9 2026. **Rule:** any new send path that creates a signed message must call `message_signing_payload()` before `sign_message()`.
