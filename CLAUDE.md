...
- Notifications system: `notification_provider.dart` (server-level All/Mentions/Nothing, per-channel override, per-DM toggle), `unread_provider.dart` (last-seen tracking, unread counts, marks seen on navigation), `system_notification_provider.dart` (`local_notifier` ^0.1.6, fires only when window unfocused + unmuted + live message, rate-limits 3+ in 5s). UI: accent dots on server strip/channel tiles/DM cards for unreads, bold text on unread items. Server Settings > Notifications tab with server-wide picker + per-channel dropdowns. DM header: member panel toggle replaced with bell icon for per-friend notification toggle.

- Notifications system: `notification_provider.dart` (server-level All/Mentions/Nothing, per-channel override, per-DM toggle), `unread_provider.dart` (last-seen tracking, unread counts, marks seen on navigation + history load), `system_notification_provider.dart` (custom in-app overlay, no `local_notifier`). UI: red pill badge with count on server strip, accent dots on channel tiles/DM cards, bold text on unread items. Muted sources hide all indicators. Custom overlay (`notification_overlay.dart`): up to 3 stacked mini-chat cards (bottom-right, `AnimatedPositioned` in `Stack`), each accumulates up to 5 messages from same source, slide-in/auto-dismiss/hover-pause/click-navigate. Server Settings > Notifications tab with server-wide picker + per-channel `PopupMenuButton`. DM header bell icon for per-friend toggle. Single-instance lock (`%APPDATA%/Haven/haven.lock`).
