...
- **Native screen share (macOS + Windows):** macOS: `FlutterScreenCaptureKitCapturer` in `packages/flutter_webrtc/macos/Classes/` feeds NV12 frames via `RTCVideoCapturerDelegate`. Windows: `WinScreenShareCapturer` in `packages/flutter_webrtc/windows/` uses BGRA direct path + custom libwebrtc DLL with `RTCVideoSource::OnCapturedFrame()`. Pipeline: WGC BGRA texture → BGRA staging → libyuv BGRA→I420 (`CreateFromBGRA`) → encoder. D3D11 Video Processor bypassed for screen share (caused dark exposure); VP still used for recording path. `ProcessAudioCapturer` (`process_audio_capturer.h/.cc`) captures system audio excluding Hollow's process via `AUDIOCLIENT_ACTIVATION_TYPE_PROCESS_LOOPBACK` — reliable on Win11, unreliable EXCLUDE mode on Win10. Audio toggle locked on Windows in UI. VP8 forced as preferred codec on all desktop platforms (was macOS-only).
- **CRITICAL — custom libwebrtc DLL:** The Windows flutter_webrtc fork requires a custom-built `libwebrtc.dll` (not the prebuilt one from flutter-webrtc releases). Built from `D:\libwebrtc-build\src\` with `ninja -C out/Windows-x64 libwebrtc:libwebrtc`. The DLL adds `OnCapturedFrame`, `CreateCustomVideoSource`, `CreateFromBGRA`, `CreateFromNV12`, and MF Hardware H.264 Encoder (`mf_video_encoder.h/.cc`, `mf_video_encoder_factory.h/.cc` in `src/libwebrtc/src/win/`). MF encoder auto-discovers NVENC/AMF/QSV but VP8 stays the default codec for screen share. The prebuilt DLL's `RTCVideoSource` is empty (no methods). `packages/flutter_webrtc/third_party/` is now tracked in git (headers, DLL, import lib, prebuilt binaries) — only backup files and download cache are gitignored.
