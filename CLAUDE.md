...
- **Identity at-rest protection:** `identity.key` encrypted with HKEYV1 format (`[magic:6][flags:1][salt:16][nonce:12][ciphertext:84]` = 119 bytes). Two mutually exclusive modes: password (flags=0x01, Argon2id + AES-256-GCM, blocks app launch with full-screen dialog) or OS keychain (flags=0x02, DPAPI on Windows / Keychain on macOS, silent unlock). Plaintext files (68 bytes, protobuf header `0x08 0x01 0x12 0x40`) auto-detected for backward compat. `unlock_identity()` must be called before any identity/DB operation — stores session wrapping key in a Rust static. All 16 `load_or_create_identity()` callsites transparently use the session key. Recovery: 24-word mnemonic restores identity and removes encryption. Files: `identity/encryption.rs` (core crypto), `identity/platform_keystore.rs` (DPAPI/Keychain), `api/identity.rs` (FFI: unlock/lock/enable/change/remove password + status). Settings → Security tab has "App Lock" section.
