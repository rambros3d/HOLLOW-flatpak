...
- **CRITICAL — MLS state must be persisted after every encrypt.** `send_mls_broadcast`, `send_mls_broadcast_topic`, and any direct `mls_mgr.encrypt()` call must call `persist_mls_state()` immediately after. MLS encrypt advances the secret tree generation — without persist, app restart reuses the old generation → `SecretReuseError` on the receiving peer. The CryptoStore actor is fire-and-forget so this is cheap.
- **CRITICAL — MLS decrypt failure must trigger immediate sync from sender.** On every decrypt failure, request `ChannelSyncRequest` from `peer_str` for subscribed channels (5s dedup via `channel_sync_sent`). This recovers dropped messages before the sender's next successful message advances per-sender timestamps past the gap. `subscribed_channels: HashMap<String, Vec<String>>` tracks topic subscriptions from `SubscribeChannels` command.
- **CRITICAL — MLS recovery (Welcome) must sync ALL channels.** After joining from Welcome, send `ChannelSyncRequest` for all server channels to the coordinator (not just empty ones). The coordinator must also request sync FROM recovered peers after batch-add. Both sides need the messages that were dropped during the stale epoch.
...
- **CRITICAL — Linux window close: minimize to taskbar, not tray.** `tray_manager` AppIndicator is broken on most Linux DEs (requires GNOME extension). `windowManager.hide()` doesn't work on Linux GTK. On Linux, `onWindowClose()` calls `minimize()`. Second close while minimized = quit (detected via `isMinimized()`). `_linuxQuit()` handles clean shutdown. Never add tray logic for Linux.
- **CRITICAL — Linux `record` package needs `parecord`.** `record_linux` shells out to `parecord` (PulseAudio CLI). Not installed on PipeWire systems. Mic test in Settings crashes on Linux. Voice calls work fine (libwebrtc talks to PipeWire directly). Needs WebRTC `getUserMedia` path for Linux mic testing.
- **Flatpak packaging:** `flatpak/` directory. `build-flatpak.sh` builds from Flutter output. Requires `--socket=x11` (not `fallback-x11`) because `main.cc` forces `GDK_BACKEND=x11`. Wrapper script sets `LD_LIBRARY_PATH`. Bundled syslibs: ayatana-appindicator, libdbusmenu. Data dir in Flatpak: `~/.var/app/com.anonlisten.Hollow/`.
