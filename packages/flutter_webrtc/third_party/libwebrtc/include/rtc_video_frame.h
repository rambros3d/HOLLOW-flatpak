...
class RTCVideoFrame : public RefCountInterface {
 ...
 public:
  ...
  // Create from BGRA pixel data (Windows Graphics Capture format).
  // Uses hardware-accelerated libyuv (AVX2/SSSE3) for conversion.
  LIB_WEBRTC_API static scoped_refptr<RTCVideoFrame> CreateFromBGRA(
      int width, int height, const uint8_t* bgra, int stride_bgra);

  // Create from NV12 biplanar data (Y plane + interleaved UV plane).
  // VP8/VP9/AV1 encoders consume NV12 directly — zero conversion.
  LIB_WEBRTC_API static scoped_refptr<RTCVideoFrame> CreateFromNV12(
