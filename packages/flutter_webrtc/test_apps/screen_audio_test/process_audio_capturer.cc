...
// --- Static support check (Windows 10 2004+ / build 19041) ---

bool ProcessAudioCapturer::IsSupported() {
  // VerifyVersionInfoW lies without an app manifest. Use RtlGetVersion instead
  // which always returns the real OS version.
...
// --- ActivationHandler (COM async callback) ---
...
ComPtr<IAudioClient> ProcessAudioCapturer::ActivateProcessLoopback(
  ...
  if (FAILED(hr)) {
    CAPLOG("ActivateAudioInterfaceAsync failed 0x%08x", hr);
    return nullptr;
  }
  ...
  if (FAILED(handler->GetActivateResult())) {
    CAPLOG("Activation result failed 0x%08x", handler->GetActivateResult());
    return nullptr;
  }
...
void ProcessAudioCapturer::CaptureThread() {
  ...
  const char* mode_str = include_mode_ ? "INCLUDE" : "EXCLUDE";
  ...
  for (UINT32 rate : rates) {
    ...
    if (!client) {
      CAPLOG("Activation failed for PID %u (%s)", target_pid_, mode_str);
      final_cleanup();
      return;
    }
    ...
    if (SUCCEEDED(hr)) {
      audio_client = client;
      chosen_rate = rate;
      CAPLOG("Initialized at %uHz (%s PID %u)", rate, mode_str, target_pid_);
      break;
    }
    CAPLOG("%uHz Initialize failed 0x%08x, trying next...", rate, hr);
  }

  if (!audio_client) {
    CAPLOG("All sample rates failed for PID %u", target_pid_);
    final_cleanup();
    return;
  }
  ...
  if (FAILED(hr)) {
    CAPLOG("SetEventHandle failed 0x%08x", hr);
  ...
  if (FAILED(hr)) {
    CAPLOG("GetService IAudioCaptureClient failed 0x%08x", hr);
    CloseHandle(capture_event);
    final_cleanup();
    return;
  }
  ...
  CAPLOG("Capturing: %s PID %u, %uHz stereo 16-bit",
         mode_str, target_pid_, chosen_rate);
