...
pub(crate) async fn start_tunnels() -> Result<Vec<tokio::task::JoinHandle<()>>, String> {
    ...
    haven_log!("[HAVEN] [PROXY] Shadowsocks tunnels started (relay=127.0.0.1:{LOCAL_RELAY_PORT}, signaling=127.0.0.1:{LOCAL_SIGNALING_PORT})");
...
async fn start_single_tunnel(
    ...
    let handle = tokio::spawn(async move {
        if let Err(e) = tunnel.run().await {
            haven_log!("[HAVEN] [PROXY] Tunnel on port {local_port} stopped: {e}");
        }
    });
